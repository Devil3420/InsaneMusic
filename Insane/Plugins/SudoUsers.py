...
from Insane import BOT_ID, BOT_USERNAME, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Insane.Database import (add_gban_user, add_off, add_on, add_sudo,
    ...
    if not message.reply_to_message:
        ...
        if user.id == from_user.id:
            ...
            m = await message.reply_text(
                f"**Insane Is Initializing Global Ban on {user.mention}**\n\nExpected Time : {len(served_chats)}"
            )
            number_of_chats = 0
    ...
    if user_id == from_user_id:
        ...
        if is_gbanned:
            ...
            for chat in chats:
                served_chats.append(int(chat["chat_id"]))
            m = await message.reply_text(
                f"**Insane Is Initializing Gobal Ban on {mention}**\n\nExpected Time : {len(served_chats)}"
            )
            number_of_chats = 0
            for sex in served_chats:
                try:
